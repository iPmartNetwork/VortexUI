      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {(packages.data?.packages ?? []).map((p) => (
          <Card
            ...
            onClick={() => { setSelected(p); setMethod(availableMethods(p)[0] ?? ""); }}
          >
        ...
        <Card className="space-y-3 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {availableMethods(selected).map((m) => (
              <Button key={m} size="sm" variant={method === m ? "primary" : "ghost"} onClick={() => setMethod(m)}>
          ...
          {method === "card_to_card" && settings && (
            <div className="rounded border bg-muted/40 p-3 text-sm space-y-1">
              ...
              <div className="font-mono">{settings.card_number}</div>
          ...
          {method === "crypto" && settings && (
            <div className="rounded border bg-muted/40 p-3 text-sm space-y-2">
              {Object.entries(settings.crypto_addresses ?? {}).map(([k, v]) => v && (
                <div key={k}><span className="text-muted-foreground">{k}:</span> <code className="text-xs">{v}</code></div>
              ))}
            </div>
          )}
          ...
          <Button onClick={submit} disabled={initDeposit.isPending || !method}>
