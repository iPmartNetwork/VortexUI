...
export function DeepLinks() {
  ...
  return (
    <div className="space-y-5 animate-page-enter">
      <div>
        <h1 className="text-2xl font-bold text-fg tracking-tight">Deep Links & QR</h1>
        <p className="text-sm text-fg-muted mt-1">One-tap subscription import via custom URL scheme</p>
      </div>

      {cfg && (
        <GlassCard hover={false} className="!p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-fg">Configuration</h3>
      ...
      <GlassCard hover={false} className="!p-5 space-y-3">
        <h3 className="text-sm font-bold text-fg flex items-center gap-1.5"><Link2 size={14} className="text-primary" /> Test Deep Link</h3>
