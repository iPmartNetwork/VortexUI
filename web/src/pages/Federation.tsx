...
export function Federation() {
  ...
  function update<K extends keyof FedConfig>(field: K, value: FedConfig[K]) {
    setForm((prev) => ({ ...(prev ?? cfgData?.config ?? ({} as FedConfig)), [field]: value }));
  }

  return (
    <div className="space-y-5 animate-page-enter">
      <div>
        <h1 className="text-2xl font-bold text-fg tracking-tight">{t("federation.title")}</h1>
        <p className="text-sm text-fg-muted mt-1">{t("federation.subtitle")}</p>
      </div>
      ...
      <div className="rounded-xl border border-primary/25 bg-primary/5 px-4 py-3 flex items-start gap-3">
        ...
        <div className="text-xs text-fg-muted leading-relaxed space-y-1.5">
          <p className="font-semibold text-fg text-sm">{t("federation.infoTitle")}</p>
          <p>{t("federation.infoDesc")}</p>
          <ul className="space-y-1 pt-1">
            <li><strong className="text-fg">Sync users</strong> — {t("federation.syncUsers")}</li>
            <li><strong className="text-fg">Sync nodes</strong> — {t("federation.syncNodes")}</li>
            <li><strong className="text-fg">SSO</strong> — {t("federation.sso")}</li>
          </ul>
        </div>
      </div>
        ...
        <GlassCard hover={false} className="!p-5 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <label className="flex items-end gap-2 text-sm text-fg pb-2">
              <input type="checkbox" checked={cfg.sso_enabled} onChange={(e) => update("sso_enabled", e.target.checked)} className="rounded" /> SSO
            </label>
          </div>
          <div className="flex justify-end pt-1 border-t border-border/40">
            <Button onClick={() => cfg && saveCfg.mutate(cfg)} disabled={saveCfg.isPending}>Save</Button>
          </div>
        </GlassCard>
      )}
      ...
      <GlassCard hover={false} className="!p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => setAddOpen(true)}>
            <Plus size={13} /> {t("federation.addPeer")}
          </Button>
        </div>
        <AddPeerModal open={addOpen} onClose={() => setAddOpen(false)} />
        <div className="space-y-2">
          {peersData?.peers?.map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-lg bg-surface-2/50 border border-border/40 px-3 py-2.5">
              ...
              <Button variant="ghost" size="sm" className="text-danger" onClick={() => delPeer.mutate(p.id)}>
                <Trash2 size={13} />
              </Button>
            </div>
          ))}
          {(!peersData?.peers || peersData.peers.length === 0) && <p className="text-sm text-fg-muted text-center py-6">{t("federation.noPeers")}</p>}
        </div>
      </GlassCard>
      ...
      <GlassCard hover={false} className="!p-4 space-y-3">
        ...
        <div className="space-y-2 max-h-[200px] overflow-y-auto">
          {eventsData?.events?.map((e) => (
            <div key={e.id} className="flex items-center justify-between text-xs rounded-lg bg-surface-2/50 border border-border/40 px-3 py-2.5">
...
function AddPeerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const create = useMutation({
    mutationFn: (b: typeof f) => api("/api/federation/peers", { method: "POST", body: b }),
    onSuccess: () => {
    ...
    <Modal open={open} onClose={onClose} title="Add Peer Panel">
      <form onSubmit={(e) => { e.preventDefault(); create.mutate(f); }} className="space-y-3">
        ...
        <div className="flex gap-4">
          <label className="flex items-center gap-2 text-sm text-fg"><input type="checkbox" checked={f.sync_users} onChange={(e) => setF((s) => ({ ...s, sync_users: e.target.checked }))} /> Sync users</label>
          <label className="flex items-center gap-2 text-sm text-fg"><input type="checkbox" checked={f.sync_nodes} onChange={(e) => setF((s) => ({ ...s, sync_nodes: e.target.checked }))} /> Sync nodes</label>
        </div>
