...
export function QuotaNotifications() {
  ...
  return (
    <div className="space-y-5 animate-page-enter">
      <div>
        <h1 className="text-2xl font-bold text-fg tracking-tight">Quota Notifications</h1>
        <p className="text-sm text-fg-muted mt-1">Notify users at each smart quota tier via Telegram or webhook</p>
      </div>

      {cfg && (
        <GlassCard hover={false} className="!p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-fg">Settings</h3>
          ...
          <div className="flex flex-wrap gap-4">
            <label className="flex items-center gap-2 text-sm text-fg"><input type="checkbox" checked={cfg.notify_telegram} onChange={(e) => update("notify_telegram", e.target.checked)} className="rounded" /> Telegram</label>
            <label className="flex items-center gap-2 text-sm text-fg"><input type="checkbox" checked={cfg.notify_webhook} onChange={(e) => update("notify_webhook", e.target.checked)} className="rounded" /> Webhook</label>
          </div>
          ...
          <div className="flex justify-end pt-1 border-t border-border/40">
            <Button onClick={() => cfg && save.mutate(cfg)} disabled={save.isPending}>Save</Button>
          </div>
        </GlassCard>
      )}

      <GlassCard hover={false} className="!p-4 space-y-3">
        <h3 className="text-sm font-bold text-fg">Recent Notifications</h3>
        <div className="space-y-2 max-h-[300px] overflow-y-auto">
          ...
          {(!eventsData?.events || eventsData.events.length === 0) && <p className="text-sm text-fg-muted text-center py-6">No notifications sent yet.</p>}
        </div>
      </GlassCard>
    </div>
  );
