...
SELECT i.id, i.node_id, i.tag, i.protocol, i.listen, i.port, i.network, i.security, i.sni, i.path, i.host, i.flow, i.evasion_profile_id, i.raw, i.enabled, i.speed_limit, i.geo_policy, n.name AS node_name
FROM inbounds i
JOIN nodes n ON n.id = i.node_id
ORDER BY n.name, i.tag
`

type ListInboundsFleetRow struct {
	Inbound  Inbound
	NodeName string
}

func (q *Queries) ListInboundsFleet(ctx context.Context) ([]ListInboundsFleetRow, error) {
	rows, err := q.db.Query(ctx, listInboundsFleet)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListInboundsFleetRow{}
	for rows.Next() {
		var i ListInboundsFleetRow
		if err := rows.Scan(
			&i.Inbound.ID,
			&i.Inbound.NodeID,
			&i.Inbound.Tag,
