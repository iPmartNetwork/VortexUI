...
SELECT i.id, i.node_id, i.tag, i.protocol, i.listen, i.port, i.network, i.security, i.sni, i.path, i.host, i.flow, i.evasion_profile_id, i.raw, i.enabled, i.speed_limit, i.geo_policy, i.core FROM inbounds i
...
const primaryInboundProtocols = `-- name: PrimaryInboundProtocols :many
...
type PrimaryInboundProtocolsRow struct {
	UserID   uuid.UUID
	Protocol string
	Network  string
	Security string
...
func (q *Queries) PrimaryInboundProtocols(ctx context.Context, userIds []uuid.UUID) ([]PrimaryInboundProtocolsRow, error) {
	rows, err := q.db.Query(ctx, primaryInboundProtocols, userIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []PrimaryInboundProtocolsRow{}
	for rows.Next() {
		var i PrimaryInboundProtocolsRow
		if err := rows.Scan(
			&i.UserID,
			&i.Protocol,
			&i.Network,
